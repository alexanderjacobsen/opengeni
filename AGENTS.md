...
- **Context overflow recovery is explicit and bounded.** Provider context-window overflows are classified inside `runAgentTurn`, force client compaction, then retry once only when no model/tool progress was persisted. If progress was persisted, the turn publishes a compacted recovery message and idles the session for the next user message. This is not a Temporal retry and must not loop.
