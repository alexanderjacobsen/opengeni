...
- **Context overflow recovery is explicit and progress-bounded.** Provider context-window overflows are classified inside `runAgentTurn`, force client compaction through the rendered-transcript summarizer + hard-trim retry + deterministic fallback ladder, then retry only after an actual compacted write when no model/tool progress was persisted. If progress was persisted, the turn auto-continues via a compaction resume notice only when the persisted active history strictly shrank; repeated productive compactions are allowed, while a no-shrink result publishes a recovery message and idles instead of churning. `compacted:false` is never a retry ticket. This is not a Temporal retry.
...
- **Control liveness is isolated from accepted host work.** The Rust supervisor answers `ping` and publishes heartbeats outside a bounded platform-work pool; a full pool returns typed, retryable `DRAINING` backpressure without marking the machine offline. A connection-generation end cancels its accepted work, native exec children are kill-on-drop, and the control plane sends an agent exec deadline inside a slightly larger request/reply deadline. Do not move host operations back into the heartbeat/subscription loop or make reconnect forget in-flight admission.
