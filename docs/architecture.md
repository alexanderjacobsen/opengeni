...
- **Control liveness is fate-isolated from host work.** The Rust supervisor never awaits platform-backed RPCs in the heartbeat/subscription loop: heartbeats are prioritized, `ping` bypasses a bounded host-work pool, and saturation returns typed retryable `DRAINING` backpressure without changing machine liveness. Admission is shared across reconnect generations; generation shutdown cancels its accepted tasks, and native exec children use kill-on-drop. The TypeScript session sends a finite agent exec deadline inside a slightly larger NATS request/reply deadline, so a caller timeout cannot leave invisible unbounded work.
