...
- **Environment variable values and per-session MCP credential headers are write-only end-to-end** — no schema ever carries a value back to a client; values are decrypted only inside the worker at sandbox/tool materialization time.
