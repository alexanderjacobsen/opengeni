...
- **A swappable compute model** — 11 sandbox backends, from local Docker to cloud providers to a user's own physical machine (bring-your-own-compute).
...
- **Data sovereignty / bring-your-own-everything.** You control your models, keys, and inference routes (OpenAI, Azure, Anthropic, Bedrock, or self-hosted) and — via the `selfhosted` backend — even the machine the agent runs on. Don't add hard dependencies on a single provider or a hosted control plane.
...
- Recoverable conditions **idle** the session (keep context) rather than fail it: retryable provider failures, the max-turns segment cap, and budget/credit exhaustion. A failed session would reject the very retry it asks for.
...
- **Environment variable values are write-only end-to-end** — no schema ever carries a value back to a client; values are decrypted only inside the worker at sandbox materialization time.

> Canonical: `packages/contracts/src/index.ts`, `packages/sdk/test/contract-parity.test.ts`.

### 3.8 Never cold-create or kill a user's self-hosted box

- A self-hosted (`selfhosted`) backend is a user's **physical machine**. **An offline agent is *not* a `NotFound`** — `isSelfhostedProviderNotFoundError` always returns `false`, because treating offline as NotFound would cold-create a rival box.
...
> Canonical: `apps/worker/src/activities/sandbox-lease.ts`, `packages/runtime/src/sandbox/selfhosted/`, [`../AGENTS.md`](../AGENTS.md).
...
- **The publishable client closure is exactly `{contracts, sdk, react}`** with zero server-internal deps; a guard test enforces this. *"If you are reading this because the guard went red: do NOT relax it."*

---

## 4. System architecture

Public clients talk only to the **Hono API**. The API authenticates/authorizes at the workspace boundary, persists durable state to **Postgres**, drives orchestration via **Temporal**, and fans out live events over **NATS Core**. A **Temporal worker** runs each agent turn as a non-retryable activity inside a **sandbox backend**.
...
3. **Create session.** `createSessionForRequest` validates payload + resources + tools + environment + model (`assertConfiguredModel` → 422 on unknown), checks usage limits/entitlements, writes durable session/goal rows + an initial event batch, enqueues the first turn, and wakes the Temporal workflow via `signalWithStart`.
...
stateDiagram-v2
  ...
  running --> idle: natural stop / max-turns / provider backpressure / budget
...
| `documents` | `@opengeni/documents` | RAG pipeline: upload → parse → chunk → embed → pgvector cosine search over document bases. | `createDocumentServices`, `indexDocumentNow`, `searchDocuments`. | `packages/documents/src/index.ts` |
...
| `react` | `@opengeni/react` | **Published.** React hooks + styled components over the SDK: live streaming, chat, timeline, sandbox surfaces, machines dashboard. | `OpenGeniProvider`, the `use*` hooks, `ChatComposer`/`MessageTimeline`/`WorkspaceDock`/etc., CSS subpath exports. | `packages/react/src/index.ts`, `packages/react/src/client.ts` |
...
| 11 | `selfhosted` | **Bring-your-own-compute** | A user's physical machine. Never cold-created/killed; `persistable:false`; controlled over NATS `ControlRpc`, not a provider API; gated OFF by default. |

**Provisioned vs self-hosted.** Provisioned backends are disposable boxes OpenGeni creates, snapshots, and reaps. Self-hosted is a first-class but fundamentally different beast — see §3.8. Capability cells are static per-backend metadata in `CAPABILITY_DESCRIPTORS` (one row per backend; `persistable ⇒ snapshot.kind !== 'none'`; `Recording.available === DesktopStream.available`).
...
> Canonical: `packages/contracts/src/index.ts` (the enum + `CAPABILITY_DESCRIPTORS`), `packages/runtime/src/sandbox/providers/index.ts` (the registry), [`../AGENTS.md`](../AGENTS.md) (Sandbox Notes).
...
- **Three memory stores + envelope.** See §3.5: `session_history_items` (model truth), `agent_run_states` (approval resume), `session_events` (audit), and `sandbox_session_envelopes` (sandbox recovery).
- **pgvector documents.** `document_bases`/`documents`/`document_chunks` with `embedding vector(3072)`. No ANN index (sequential scan); embedding model is stored per chunk and filtered on at search time.
...
| Documents / RAG | `packages/documents/src/index.ts` | — |
