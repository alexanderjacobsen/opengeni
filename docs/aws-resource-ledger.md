...
| --- | --- | --- | --- | --- | --- | --- |
| deleted | AWS managed live verification stack | Verification account and region | Live managed-stack conformance across EKS, ECR, RDS PostgreSQL, S3, Secrets Manager, IAM/IRSA, upstream NATS/Temporal charts, OpenGeni Helm, auth, and teardown evidence | `deploy/terraform/aws`, `deploy/stacks`, Helm, and conformance scripts | Destroyed after user-approved teardown on 2026-05-24 | Helm releases and namespaces were removed, Terraform destroy completed, the runtime secret was force-deleted, and follow-up AWS checks found no matching EKS cluster, ECR repositories, RDS instance, S3 bucket, VPC, IAM roles, or active Secrets Manager secret. Exact private cleanup evidence remains in `.agent/cloud-resource-ledger.md` and `.agent/progress.md`. |
