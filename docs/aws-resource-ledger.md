...
| --- | --- | --- | --- | --- | --- | --- |
| active | AWS managed live verification stack | Verification account and region | Live managed-stack conformance across EKS, ECR, RDS PostgreSQL, S3, Secrets Manager, IAM/IRSA, upstream NATS/Temporal charts, OpenGeni Helm, auth, and teardown evidence | `deploy/terraform/aws`, `deploy/stacks`, Helm, and conformance scripts | Retain until conformance evidence is captured, then destroy via private Terraform state and Helm cleanup commands | Created for the 2026-05-20 open-source deployment-platform verification. Exact account id, resource prefix, state path, and private cleanup commands are tracked outside public docs in `.agent/cloud-resource-ledger.md`. |
