// One-shot generator for the "screenshot omitted" PNG card that image-history.ts
// substitutes for an ELIDED (not failed) computer_call_output screenshot - see
// collectComputerOutputImages in packages/runtime/src/image-history.ts. The hosted
// computer_use_preview protocol requires computer_call_output.output.image_url to be
// a syntactically valid URL; the text placeholder used for tool-result elision
// (SCREENSHOT_OMITTED_PLACEHOLDER) is NOT a valid URL there, so it 400s the request
// the moment a 4th screenshot gets elided from history. This card is the
// image-channel equivalent of that text placeholder.
//
// Run: `bun run scripts/gen-screenshot-omitted-card.mjs` to print the data-URI base64
// (bake it into screenshot-omitted-card.ts) plus an ASCII preview for visual
// verification. The output is a valid 8-bit RGBA PNG; validate it independently
// before baking.
//
// Dependency-free (embedded 5x7 bitmap font + hand-rolled PNG via node:zlib), so
// the runtime never needs a canvas/image dependency. Font is a copy of the one in
// gen-screenshot-error-card.mjs (kept duplicated rather than shared so each
// generator stays a single self-contained file).
...
// Every character used below is a glyph the FONT table above defines.
const LINES = [
  "SCREENSHOT OMITTED",
  "EARLIER FRAME ELIDED",
  "TO SAVE CONTEXT.",
];

const SCALE = 4; // px per font pixel
const CHAR_GAP = 1; // font px between chars
const MARGIN = 24; // px
const LINE_GAP = 6; // font px between lines
const BG = [24, 20, 28, 255]; // charcoal
const HEADER_BG = [30, 60, 120, 255]; // slate-blue header band (informational, not an error)
