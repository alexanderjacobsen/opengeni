...
async function login(): Promise<{ accessToken: string; chatgptAccountId: string | null; isFedramp: boolean; plan: string | null }> {
  ...
  for (;;) {
    ...
    const poll = await pollDeviceCode({ deviceAuthId: start.deviceAuthId, userCode: start.userCode });
    if (poll.status === "pending") { process.stdout.write("."); continue; }
    if (poll.status === "expired") throw new Error("device code expired — re-run");
    const tokens = await exchangeDeviceCode({ authorizationCode: poll.authorizationCode, codeVerifier: poll.codeVerifier });
    ...
    return { accessToken: tokens.accessToken, chatgptAccountId: id.chatgptAccountId, isFedramp: id.isFedramp, plan: id.planType };
  }
}

async function streamOneTurn(auth: { accessToken: string; chatgptAccountId: string | null; isFedramp: boolean }, model: string): Promise<void> {
  ...
  if (!res.ok || !res.body) { console.log("  body:", (await res.text()).slice(0, 600)); return; }
...
async function main(): Promise<void> {
  ...
  console.log(`  ok=${models.ok} status=${models.status} slugs=${models.slugs.slice(0, 12).join(", ")}${models.slugs.length > 12 ? " …" : ""}`);
